...
const PlayingReview = () => {
  ...
  useEffect(() => {
    const fetchShop = async () => {
      try {
        const res = await fetch("http://localhost:8000/playing");
    ...
    const fetchReviews = async () => {
      try {
        const res = await fetch(`http://localhost:8000/playing/${storeId}/review`);
        const data = await res.json();
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      ...
      <img
        src={`/assets/혼놀/${shop.storeid}.jpg`}
        alt={shop.storename}
      ...
      <p className="text-gray-500 mb-4">⭐ {shop.rating} / 혼놀 점수 {shop.honbab_cnt}</p>
